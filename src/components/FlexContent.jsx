...
const FlexContent = ({ifExist, endpoint:{heading, title,text,url,btn,img}}) => {
  return (
    <>
      ...
      ${ifExist ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full mt-2'>
          <img src={img} alt={`img/${heading}`} 
          className={`w-auto object-fill transitions-theme 
          ${ifExist ? 'h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' 
